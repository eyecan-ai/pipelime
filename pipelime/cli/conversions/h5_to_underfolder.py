...
@click.option('-i', '--input_filename', required=True, type=str, help='Input H5 Filename')
@click.option('-o', '--output_folder', required=True, type=str, help='Output Underfolder')
@click.option('-r', '--reference_folder', default='', type=str, help='Reference Underfolder to retrieve extensions and rootfiles')
@click.option('-c', '--copy_root_files', default=True, type=bool, help='Copy root files')
def h5_to_underfolder(input_filename, output_folder, reference_folder, copy_root_files):
    ...
    reader = H5Reader(filename=input_filename, copy_root_files=copy_root_files, lazy_samples=True)
