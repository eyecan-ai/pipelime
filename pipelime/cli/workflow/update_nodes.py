    ...
    for name, node in nodes.items():
        if name in answers['nodes']:
            ...
            update_answers = inquirer.prompt([
                inquirer.Text('name', message='Update name', default=node.name, validate=lambda _, x: x not in available_names),
                inquirer.Text('script', message='Update script', default=node.cwl_template.script),
                inquirer.Text('alias', message='Update alias', default=" ".join(node.cwl_template.alias)),
                inquirer.Checkbox('forwards', message='Update forwards', choices=node.cwl_template.inputs_keys, default=node.cwl_template.forwards)
            ])
            ...
            CwlNodesManager.create_node(update_answers['name'], cwl_template=cwl_template, folder=folder)
