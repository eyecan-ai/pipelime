    ...
    def generate_plain_samples(
        ...
        for i in range(N):
            data = {
                ...
                'data1': np.random.uniform(0, 255, (8, 8)).astype(np.uint8) if heavy_data else 25588,
                'metadata': {
                    'even': i % 2 == 0,
                    'name': f'{namespace}{i}',
                    'N': i,
...
def plain_samples_sequence_generator(plain_samples_generator):
    def generate_plain_samples_sequence(
        ...
        return SamplesSequence(samples=plain_samples_generator(namespace, N, group_each, heavy_data))
